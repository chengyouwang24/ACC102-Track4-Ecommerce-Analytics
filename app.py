...
st.set_page_config(page_title="Online E-commerce Analytics", layout="wide", page_icon="🛒")
st.title("🛒 Online E-commerce Full Operation Analytics System")
st.subheader("All-in-One Data Platform for Online Sellers | Boost Profit | Lower Risk")
...
# 【核心新增】给月度数据计算3个核心比率指标（解决KeyError，切换年份实时更新）
if not df_month_filter.empty:
    # 退货率 = 退货金额 / 总销售额 *100
    df_month_filter["Return_Rate"] = np.where(
        df_month_filter["Total_Sales"]>0,
        abs(df_month_filter["Returns"])/df_month_filter["Total_Sales"]*100,
        0
    ).round(2)
    # 折扣率 = 折扣金额 / 总销售额 *100
    df_month_filter["Discount_Rate"] = np.where(
        df_month_filter["Total_Sales"]>0,
        abs(df_month_filter["Discounts"])/df_month_filter["Total_Sales"]*100,
        0
    ).round(2)
    # 利润率 = 净销售额 / 总销售额 *100
    df_month_filter["Profit_Margin"] = np.where(
        df_month_filter["Total_Sales"]>0,
        df_month_filter["Net Sales"]/df_month_filter["Total_Sales"]*100,
        0
    ).round(2)
...
# -------------------------- 全局核心数据预计算（杜绝所有未定义报错） --------------------------
# 1. 全局经营KPI
total_gross_sales = df_month_filter["Total_Sales"].sum()  # 原Gross Sales已被重命名为Total_Sales
total_discount_loss = abs(df_month_filter["Discounts"].sum())
total_return_loss = abs(df_month_filter["Returns"].sum())
total_net_sales = df_month_filter["Net Sales"].sum()
total_orders = df_month_filter["Total_Orders"].sum()
avg_aov = df_month_filter["AOV"].mean()
avg_return_rate = df_month_filter["Return_Rate"].mean()
avg_discount_rate = df_month_filter["Discount_Rate"].mean()
avg_margin = df_month_filter["Profit_Margin"].mean()
